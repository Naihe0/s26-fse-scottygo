...
Remaining verification: root owns browser layout checks at narrow and wide viewports and deployment. The separate filter-controller health poll is outside this change; selected-route and directions vehicle polling is paused in the background. Provider freshness remains based on the existing backend health contract plus browser comparison of vehicle timestamps; no new backend endpoint or data migration was introduced.
