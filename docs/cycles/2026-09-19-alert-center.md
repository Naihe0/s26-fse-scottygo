...
With portable Node 24.20.0, the focused `alert-content.test.ts` and `notification-rendering.test.ts` suites passed **37 tests**. They cover unsafe content/protocols, punctuation and provider separator cases, encoded route actions, recognized metadata, date handling, long-description disclosures, exact prefills, local filtering, partial failure, older-response ownership, focus/disclosure preservation, time updates, hidden-page polling, and BFCache restoration. Owned TypeScript ESLint checks and the full client TypeScript check passed.

Hosted cold-start acceptance identified a misleading zero-result message when one source returned 503 and the other successfully returned an empty list. The follow-up makes All-view empty/search copy explicitly describe only available results and mention unavailable sources. A healthy Live-only view retains its accurate rider-specific empty message when agency alerts are unavailable. Three regressions cover both directions of partial source failure and the healthy Live-only case.
