...
- Final application commit `4b303a1` auto-deployed successfully. The live map was opened while `/transit/health` reported `gtfs.ready: false`: it displayed the loading message with no stop markers, then removed the message and populated **106 stop markers without a browser reload** when the timetable became ready. All **14 hosted smoke checks passed again**. Selecting a stop on route 61C and requesting directions produced a 21-minute walking route; Exit restored the route view and the transit-system filter opened normally. Local app and MongoDB processes, including disposable audit services, are stopped.
